...
struct schedule_unit_description_element *total_schedule_unit_array;//姑且称作无敌调度大数组
int total_schedule_unit_num;

struct sub_cluster_rank_array_element *sub_cluster_rank_array;//用于选取集群，临时一用，用于排序
int sub_cluster_rank_array_num;

void master_scheduler_init();
void select_schedule_unit(void);
void add_job_schedule_unit(struct job_description_element *job_description);
int can_schedule_normal_sub_task(struct prime_job_description_element *prime_job_description,int sub_task_index);
void try_add_schedule_unit_sub_pack(struct sub_pack_description_tree_element *root,int job_id,int n_id[10],int n);
void add_schedule_unit_sub_pack(struct sub_pack_description_tree_element *root,int job_id,int n_id[10],int n);
void add_schedule_unit_normal_sub_task(struct job_description_element *job_description,int normal_sub_task_index);
void reset_total_schedule_unit(void);
void assign_schedule_unit(void);
void change_schedule_unit_status_to_running(int schedule_unit_index,int selected_sub_cluster_id);
...
void change_sub_pack_priority(struct sub_pack_description_tree_element *t,int priority);
int construct_DAG(struct job_description_element *t);
void select_start_node(int *start_node);
int calc_longest_path(int start_node_index);
void init_sub_cluster_rank_array();
int select_sub_cluster(int schedule_unit_index);
int try_to_create_sub_cluster(int need_machine_num,int schedule_unit_index);
